...
until [ $sleep_time -eq 8 ]; do
    sleep $(( sleep_time++ ))
    OSLIVELY_TEST_ETCD_HOST=$(sudo rkt list | grep etcd | cut -f7 | cut -d'=' -f2)
    if [[ "$OSLIVELY_TEST_ETCD_HOST" != "" ]]; then
        echo "ok."
        break
    fi
done
echo "etcd running in container at ${OSLIVELY_TEST_ETCD_HOST}."
export OSLIVELY_TEST_ETCD_HOST
echo "exported OSLIVELY_TEST_ETCD_HOST"
